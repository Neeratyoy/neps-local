...
# These classes are heavily inspired (i.e. pretty much a copy-paste) of HPBandSter
# (https://github.com/automl/HpBandSter) and the design principles used in BOHB:
# Robust and Efficient Hyperparameter Optimization at Scale


class KDEKernel:

    def __init__(self, data=None, bandwidth=None, fix_boundary=False, num_values=None, log=False):
...
class Gaussian(KDEKernel):
    ...
    def _compute_weights(self):
        ...
        for i, d in enumerate(self.data):
            weights[i] = 2.0 / (
                erf((1 - d) / (np.sqrt(2) * self.bw)) +
                erf(d / (np.sqrt(2) * self.bw))
            )
    ...
    def __call__(self, x_test):
        ...
        pdfs = np.exp(-0.5 * np.power(distances / self.bw, 2)) / (
            2.5066282746310002 * self.bw
        )
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        samples += delta
        oob_idx = np.argwhere(np.logical_or(
            samples > 1, samples < 0)).flatten()
        ...
        while len(oob_idx) > 0:
            ...
            oob_idx = oob_idx[
                np.argwhere(np.logical_or(
                    samples[oob_idx] > 1, samples[oob_idx] < 0))
            ].flatten()

        return samples


class AitchisonAitken(KDEKernel):
...
class WangRyzinOrdinal(KDEKernel):
    ...
    def _compute_weights(self):
        ...
        weights = 1.0 / pdfs.sum(axis=1)[:, None]
        return weights
    ...
    def __call__(self, x_test):
        ...
        idx = np.abs(distances) < 0.1

        pdfs = np.zeros_like(distances, dtype=np.float642)
        pdfs[idx] = 1 - self.bw
        pdfs[~idx] = 0.5 * (1 - self.bw) * \
            np.power(self.bw, np.abs(distances[~idx]))
        # reweigh to compensate for boundaries
        pdfs *= self.weights
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        while len(oob_idx) > 0:
            samples[oob_idx] -= delta[oob_idx]  # revert move
            delta[oob_idx] = np.random.choice(
                possible_steps, size=len(oob_idx), p=ps)
            samples[oob_idx] += delta[oob_idx]
            # import pdb; pdb.set_trace()
            oob_idx = oob_idx[
                np.argwhere(
                    np.logical_or(
                        samples[oob_idx] > self.num_values -
                        0.9, samples[oob_idx] < -0.1
                    )
                ).flatten()
            ]
...
class WangRyzinInteger(KDEKernel):
    
    def _compute_weights(self):
        if not self.fix_boundary:
            return 1.0
        weights = np.zeros(self.data.shape[0], dtype=np.float64)
        x_test = np.linspace(
            1 / (2 * self.num_values),
    ...
    def __call__(self, x_test):
        ...
        pdfs = np.zeros_like(distances, dtype=np.float64)

        idx = np.abs(distances) < 1 / (
            3 * self.num_values
        )  # distances smaller than that are considered zero
        pdfs[idx] = 1 - self.bw
        pdfs[~idx] = (
            0.5
            * (1 - self.bw)
            * np.power(self.bw, np.abs(distances[~idx]) * self.num_values)
        )
        # reweigh to compensate for boundaries
        pdfs *= self.weights
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        possible_steps = (
            np.arange(-self.num_values + 1, self.num_values) / self.num_values
        )
        ps = 0.5 * (1 - self.bw) * np.power(self.bw, np.abs(possible_steps))
        ...
        while len(oob_idx) > 0:
            ...
            oob_idx = oob_idx[
                np.argwhere(
                    np.logical_or(
                        samples[oob_idx] > 1 - 1 / (3 * self.num_values),
                        samples[oob_idx] < 1 / (3 * self.num_values),
                    )
                ).flatten()
            ]
