...
class SuccessiveHalving(BaseOptimizer):
    """Implements a SuccessiveHalving procedure with arbitrary sampling and promotion policies."""
            ...
            if top_k > 0:
                if self.promotion_policy is None:
                    self.rung_promotions[_rung] = np.array(self.rung_members[_rung])[
                        np.argsort(self.rung_members_performance[_rung])[
                            :top_k]].tolist()
                else:
                    promotion_kwargs = {}
                    self.rung_promotions[_rung] = self.promotion_policy.promote(
                        k, self.rung_members[_rung], self.rung_members_performance[_rung], **promotion_kwargs).tolist()
